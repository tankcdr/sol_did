...
impl DidAccount {
    ...
    pub fn authority_key(&self) -> Pubkey {
        let key_array: [u8; 32] = self.initial_verification_method.key_data[..32].try_into().unwrap();
        Pubkey::from(key_array)
    }
    ...
    pub fn set_native_controllers(&mut self, native_controllers: Vec<Pubkey>) -> Result<()> {
        ...
        let own_authority = Pubkey::from(<[u8; 32]>::try_from(&self.initial_verification_method.key_data[..32]).unwrap());
